      ...
      <div className="w-full max-w-md rounded-2xl overflow-hidden shadow-2xl"
        ...
        {status === 'success' ? (
          ...
          <div className="px-6 py-5 space-y-5">
            {/* Section 1 — Send to self */}
            <div>
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className="relative">
                  <input type="checkbox" checked={sendToSelf}
                  ...
                  <div className="w-4 h-4 rounded flex items-center justify-center transition-all"
                    ...
                    {sendToSelf && <svg className="w-2.5 h-2.5" viewBox="0 0 10 8" fill="none">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-lg text-sm text-text-primary outline-none transition-all"
            ...
            {/* Section 3 — Note */}
            <div>
              <div className="text-xs font-semibold uppercase tracking-wider text-text-muted mb-2">Add a Note <span className="normal-case font-normal">(optional)</span></div>
              <textarea
                value={note}
                onChange={e => setNote(e.target.value)}
                rows={3}
                placeholder="e.g. Please review before the pre-bid meeting on [date]"
                className="w-full px-3 py-2.5 rounded-lg text-sm text-text-primary outline-none transition-all resize-none"
                style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#F5F9FF' }}
                onFocus={e => { e.currentTarget.style.borderColor = 'rgba(0,229,255,0.4)'; }}
                onBlur={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; }}
              />
