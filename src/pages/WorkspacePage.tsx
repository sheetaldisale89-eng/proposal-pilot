...
function cleanTitle(title: string): string {
  ...
  // If it looks like a raw filename (has underscores/hyphens but no spaces, or ends in .pdf)
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [{ data: projectsData }, { data: analysisData }, { data: userData }] = await Promise.all([
        ...
        supabase.from('ai_analysis_results').select('project_id, status, full_analysis_json, confidence_score'),
      ...
      // Build a map of project_id → go_no_go recommendation
      ...
      const completedCount = (analysisData ?? []).filter(a => a.status === 'completed').length;
      const thisMonthCount = rawProjects.filter(p => new Date(p.created_at) >= monthStart).length;
      const distinctInstitutions = new Set(rawProjects.map(p => p.client_name).filter(Boolean)).size;
        ...
        <div className="p-8 space-y-8">
                ...
                <div key={s.label} className="rounded-xl p-5 flex items-start justify-between transition-all"
                  ...
                  <div>
                    <div className="font-bold text-3xl font-mono mb-1" style={{ color: s.color }}>
                      {loading ? <div className="w-8 h-7 rounded animate-pulse" style={{ background: 'rgba(255,255,255,0.08)' }} /> : s.value}
                    </div>
                    <div className="text-[11px] text-text-muted uppercase tracking-wider">{s.label}</div>
                  </div>
                  <Icon className="w-4 h-4 mt-1" style={{ color: 'rgba(156,174,196,0.4)' }} />
                </div>
              );
            })}
          </div>
          ...
          <div>
            ...
            {!loading && sortedProjects.length === 0 && (
              <div className="rounded-2xl py-20 text-center" style={{ background: '#08111F', border: '1px solid rgba(255,255,255,0.06)' }}>
                                ...
                                <tr key={row.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: 'rgba(255,77,109,0.04)' }}>
                                  <td colSpan={4} className="px-6 py-4">
                                    <div className="flex items-center gap-4">
                                      <span className="text-sm text-text-secondary">Delete <span className="font-semibold text-text-primary">"{cleanTitle(row.title)}"</span>? This cannot be undone.</span>
                                      <button
                                ...
                                <td className="px-6 py-4">
                                  <div className="flex items-center justify-end gap-1.5">
                                    ...
                                    <ActionBtn
                                      ...
                                      tooltip="View Intelligence Brief"
                                      hoverColor="#00E5FF"
                                    ...
                                    <ActionBtn
                                      ...
                                      tooltip="Email Intelligence Brief"
                                      hoverColor="#00E5FF"
                                      onClick={() => setEmailProjectId(row.id)}
                                    />
                                    {/* Download */}
                                    <ActionBtn
                                      icon={downloadingId === row.id
  ...
  return (
    <div className="relative group">
      <button
        ...
        style={{
          background: hovered ? `rgba(${hoverColor === '#FF4D6D' ? '255,77,109' : '0,229,255'},0.1)` : 'rgba(255,255,255,0.04)',
          border: `1px solid ${hovered ? (hoverColor === '#FF4D6D' ? 'rgba(255,77,109,0.25)' : 'rgba(0,229,255,0.25)') : 'rgba(255,255,255,0.07)'}`,
          color: hovered ? hoverColor : '#9CAEC4',
        }}>
        {icon}
      </button>
      {/* Tooltip */}
      <div className="absolute bottom-full right-0 mb-1.5 px-2 py-1 rounded text-[10px] whitespace-nowrap pointer-events-none transition-opacity z-30"
        style={{ background: '#0D1829', border: '1px solid rgba(255,255,255,0.1)', color: '#F5F9FF', opacity: hovered ? 1 : 0 }}>
        {tooltip}
      </div>
    </div>
  );
